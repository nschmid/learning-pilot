<x-guest-layout>
    <x-authentication-card>
        ...
        <div class="text-center mb-8">
            <h1 class="text-2xl font-bold text-gray-900">{{ __('Konto erstellen') }}</h1>
            <p class="mt-2 text-sm text-gray-600">{{ __('Starten Sie Ihre Lernreise noch heute') }}</p>
        </div>
            ...
            @if (Laravel\Jetstream\Jetstream::hasTermsAndPrivacyPolicyFeature())
                <div class="flex items-start">
                    ...
                    <label for="terms" class="ml-2 text-sm text-gray-600">
                        {!! __('Ich akzeptiere die :terms_of_service und die :privacy_policy', [
                            'terms_of_service' => '<a target="_blank" href="'.route('terms.show').'" class="font-medium text-teal-600 hover:text-teal-500">'.__('AGB').'</a>',
                            'privacy_policy' => '<a target="_blank" href="'.route('policy.show').'" class="font-medium text-teal-600 hover:text-teal-500">'.__('Datenschutzerklärung').'</a>',
                        ]) !!}
                    </label>
                </div>
            @endif
